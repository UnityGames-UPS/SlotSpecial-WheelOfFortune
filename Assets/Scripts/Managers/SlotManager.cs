...
public class SlotManager : MonoBehaviour
{
  ...
  [SerializeField] private ImageAnimation thirdColAnticipationAnim; // single merged, one-shot sequence that gates the reel-3 stop; idles on the default reel bg sprite
  internal bool playThirdColAnticipation = false;
  ...
  {
    float delay = 0.55f;
    int lastIndex = alltweens.Count - 1;
    bool didAnticipate = false;

    for (int i = 0; i < alltweens.Count; i++)
    {
      // Once columns 1 and 2 have stopped, run the anticipation on the third (last) column:
      // keep it spinning while the merged border/divider animation plays. We DON'T wait for
      // the animation to fully finish here — we wait until it's within one reel-stop duration
      // of ending, then let the reel-3 stop tween below run concurrently with its tail so the
      // animation and the reel settle at nearly the same time (avoids a snappy sequential stop).
      // Skipped if the player already forced an immediate stop before reaching reel 3.
      if (i == lastIndex && playThirdColAnticipation && !gameManager.immediateStop
          && thirdColAnticipationAnim != null)
      {
        didAnticipate = true;
        StartAnticipation();
        yield return new WaitUntil(() =>
          gameManager.immediateStop
          || !thirdColAnticipationAnim.isplaying
          || thirdColAnticipationAnim.RemainingSeconds <= delay);
      }
    ...
    if (didAnticipate) StopAnticipation();
    alltweens.Clear();
    playThirdColAnticipation = false;
    // KillAllTweens();
  }
  ...
  {
    if (thirdColAnticipationAnim == null) return;
    thirdColAnticipationAnim.doLoopAnimation = false;
    thirdColAnticipationAnim.StopAnimation(); // reset state so it replays cleanly each spin
    thirdColAnticipationAnim.StartAnimation();
    if (audioController) audioController.PlayThirdColSpinAudio();
  }

  private void StopAnticipation()
  {
    if (audioController) audioController.StopThirdColSpinAudio();
    if (thirdColAnticipationAnim == null) return;
    thirdColAnticipationAnim.StopAnimation();
    // Object stays active as the reel background, so restore its default bg sprite
    // instead of leaving the animation's first frame showing.
    thirdColAnticipationAnim.RestoreOriginalSprite();
  }
